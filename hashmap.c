...
map_t hashmap_new(unsigned long size) {
    ...
    m->hash_fun = HASHFUN; // Set hash function based on configured one in header
	if(!m) goto err; // error allocating map
	...
	if(!m->data) goto err; // error allocating map data
...
unsigned int hashmap_hash_int_diff(hashmap_map *m, char *keystring) {
    unsigned int key = m->hash_fun(keystring);
...
int hashmap_put(map_t in, char* key, any_t value) {
    ...
    // overwrite map value if it exists, and don't increase the size
    if(m->data[index].in_use == 1) {
        // do not increase map size
    } else {
        m->size++;
    }
	/* Set the data */
